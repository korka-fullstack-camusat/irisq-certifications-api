...
from routes import forms, responses, upload, exams, auth, sessions, candidate, candidate_auth, candidat_account
...
async def startup_event():
    ...
    console.print(Panel.fit(Text("IRISQ API is starting \U0001f680", justify="center", style="bold green"), border_style="green"))


@app.on_event("shutdown")
async def shutdown_event():
    await close_mongo_connection()
    console.print(Panel.fit(Text("IRISQ API is shutdown \U0001f6d1", justify="center", style="bold red"), border_style="red"))
...
app.include_router(candidat_account.router, prefix="/api/account", tags=["Candidate Account"])
